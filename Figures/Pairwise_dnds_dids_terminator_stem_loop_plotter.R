...
file_base_1="/Analysis/Pairwise_dnds_dids_terminator_stem_loop/"
...
for(i in 1:species_count){
  ...
  dnds_dids_terminator_stem_loop_data <- cbind(Species, dnds_dids_terminator_stem_loop_data)
  
  print(wilcox.test(dnds_dids_terminator_stem_loop_data$dI.dS_terminator_stem, dnds_dids_terminator_stem_loop_data$dI.dS_terminator_loop, alternative="less"))
  
  species_dnds_dids_terminator_stem_loop_data <- rbind(species_dnds_dids_terminator_stem_loop_data, dnds_dids_terminator_stem_loop_data)
}

species_dnds_dids_terminator_stem_loop_data_long <- melt(species_dnds_dids_terminator_stem_loop_data, measure.vars=c("dN.dS", "dI.dS_terminator_stem", "dI.dS_terminator_loop"), variable.name="Category", value.name="dX.dS")

species_dnds_dids_terminator_stem_loop_data_long$dS_bin <- cut(species_dnds_dids_terminator_stem_loop_data_long$dS, breaks=seq(0, 0.1, 0.0001), labels=seq(0.0001, 0.1, 0.0001))

species_dnds_dids_terminator_stem_loop_data_long_summary <- summarySE(species_dnds_dids_terminator_stem_loop_data_long, measurevar="dX.dS", groupvars=c("Species", "Category", "dS_bin"))
...
category_breaks=c("dN.dS", "dI.dS_terminator_stem", "dI.dS_terminator_loop")
...
#out_file_pdf <- paste(base_dir, "/Figures/Figure_S7", ".pdf", sep="")
out_file_tif <- paste(base_dir, "/Figures/Figure_S7", ".tif", sep="")

#pdf(file=out_file_pdf, height=10, width=15)
tiff(file=out_file_tif, height=10, width=15, units="in", res=100)

pairwise_dnds_dids_terminator_stem_loop_plot
