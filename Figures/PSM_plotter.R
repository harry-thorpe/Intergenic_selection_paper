...
for(i in 1:species_count){
  ...
  resampled_PSM_data <- data.frame(Species, Synonymous, Intergenic, Nonsynonymous, Nonsense)
  
  species_PSM_data <- rbind(species_PSM_data, resampled_PSM_data)
}

species_PSM_data <- melt(species_PSM_data, id.vars=c("Species"), variable.name="Category", value.name="PSM")
...
species_PSM_data_summary <- summarySE(species_PSM_data, groupvars=c("Species", "Category", "Order"), measurevar="PSM")
...
PSM_plot <- ggplot(species_PSM_data_summary, aes(x=reorder(Category, Order), y=PSM, fill=Category)) +
  geom_bar(stat="identity") +
  geom_errorbar(aes(x=reorder(Category, Order), ymin=PSM-se, ymax=PSM+se), width=0.5) +
