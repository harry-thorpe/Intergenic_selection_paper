...
$in_int_all_file="${base_dir}/Analysis/Core_genome_alignment/${species}_Core_genome_alignment/${species}_core_intergenic_alignment.fasta";
$in_int_ups_file="${base_dir}/Analysis/Pairwise_dnds_dids_upstream/${species}_Pairwise_dnds_dids_upstream/${species}_core_intergenic_alignment_upstream_30.fasta";

open OUTPUT_GENE, ">${base_dir}/Analysis/${analysis}/${species}_$analysis/${species}_core_gene_alignment_simulated.fasta";
open OUTPUT_INT_ALL, ">${base_dir}/Analysis/${analysis}/${species}_$analysis/${species}_core_intergenic_all_alignment_simulated.fasta";
open OUTPUT_INT_UPS, ">${base_dir}/Analysis/${analysis}/${species}_$analysis/${species}_core_intergenic_upstream_30_alignment_simulated.fasta";
...
open INPUT, $in_int_all_file;
while(<INPUT>){
	if(/^([ATGCN]+)/){
		$int_all_seq=$1;
		$int_all_len=length($int_all_seq);
		last;
	}
}
open INPUT, $in_int_ups_file;
while(<INPUT>){
	if(/^([ATGCN]+)/){
		$int_ups_seq=$1;
		$int_ups_len=length($int_ups_seq);
		last;
	}
}

$seq="$gene_seq$int_all_seq$int_ups_seq";
$tsites=length($seq);
...
foreach $max_muts(@max_muts_array){
	for $y(1..$strs){
		...
		$min=$max;
		$max=$max+$int_all_len;
		print OUTPUT_INT_ALL ">${y}_$max_muts\n";
		for ($i=$min; $i<$max; $i++){
			print OUTPUT_INT_ALL "$mutseq[$i]";
		}
		print OUTPUT_INT_ALL "\n";
		
		$min=$max;
		$max=$max+$int_ups_len;
		print OUTPUT_INT_UPS ">${y}_$max_muts\n";

		for ($i=$min; $i<$max; $i++){
			print OUTPUT_INT_UPS "$mutseq[$i]";
		}
		print OUTPUT_INT_UPS "\n";
	}
}
