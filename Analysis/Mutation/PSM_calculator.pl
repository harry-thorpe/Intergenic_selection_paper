...
foreach $codon_pos(@biallelic_codon_array){
	...
	if($codon_count == 2){
		...
		if($codon_snp == 1){
			...
			print OUTPUT_2 "$real_base_pos\t$mut\t$codon_array[0]\t$codon_array[1]\t$codon_hash{$codon_pos}{$codon_array[0]}\t$codon_hash{$codon_pos}{$codon_array[1]}\t$codon_isolate_hash{$codon_pos}{$codon_array[0]}\t$codon_isolate_hash{$codon_pos}{$codon_array[1]}\n";
...
foreach $base_pos(@biallelic_base_array){
	...
	if($base_count == 2){
		...
		print OUTPUT_2 "$real_base_pos\t$mut\t$base_array[0]\t$base_array[1]\t$base_hash{$base_pos}{$base_array[0]}\t$base_hash{$base_pos}{$base_array[1]}\t$base_isolate_hash{$base_pos}{$base_array[0]}\t$base_isolate_hash{$base_pos}{$base_array[1]}\n";
...
foreach $category(@category_array){
	...
	print OUTPUT "$category,$mut_type_hash{$category},$mut_type_singleton_hash{$category},$PSM\n";
}
