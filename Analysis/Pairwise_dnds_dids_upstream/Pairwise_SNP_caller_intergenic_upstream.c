...
char upstream_len[arg_len];
strcpy(upstream_len, argv[4]);
...
strcat(output_file_name, "/");
strcat(output_file_name, species);
strcat(output_file_name, "_di_upstream_");
strcat(output_file_name, upstream_len);
...
if(isolates_file_2 != NULL){
	while(fgets(isolate_name, sizeof isolate_name, isolates_file_2) != NULL){
		...
		strcat(isolate_file_name_array[isolate_count], analysis);
		strcat(isolate_file_name_array[isolate_count], "/Intergenic_files_upstream_");
		strcat(isolate_file_name_array[isolate_count], upstream_len);
		strcat(isolate_file_name_array[isolate_count], "/");
...
printf ("%s di upstream %s calculated.\n", species, upstream_len);

fprintf (log_file, "%s di upstream %s calculated.\n", species, upstream_len);
