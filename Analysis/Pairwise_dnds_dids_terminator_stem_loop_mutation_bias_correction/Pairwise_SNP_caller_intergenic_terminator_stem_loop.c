...
char category[arg_len];
strcpy(category, argv[4]);

char category_file[arg_len];
strcpy(category_file, argv[5]);
...
strcat(output_file_name, species);
strcat(output_file_name, "_");
strcat(output_file_name, category_file);
strcat(output_file_name, "_di_simulated.csv");
...
strcat(isolates_file_name, "/Analysis/");
strcat(isolates_file_name, analysis);
strcat(isolates_file_name, "/");
strcat(isolates_file_name, species);
strcat(isolates_file_name, "_");
strcat(isolates_file_name, analysis);
strcat(isolates_file_name, "/");
strcat(isolates_file_name, "Isolates.txt");
...
if(isolates_file_2 != NULL){
	while(fgets(isolate_name, sizeof isolate_name, isolates_file_2) != NULL){
		...
		strcat(isolate_file_name_array[isolate_count], analysis);
		strcat(isolate_file_name_array[isolate_count], "/Intergenic_");
		strcat(isolate_file_name_array[isolate_count], category_file);
		strcat(isolate_file_name_array[isolate_count], "_files/");
...
printf ("%s intergenic annotation %s di calculated.\n", species, category_file);

fprintf (log_file, "%s intergenic annotation %s di calculated.\n", species, category_file);
