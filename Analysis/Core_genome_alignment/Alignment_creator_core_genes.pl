...
foreach $threshold(@threshold_array){
	if($threshold == 95){
		$threshold_folder="";
	}else{
	...
	open OUTPUT, ">${base_dir}/Analysis/${analysis}/${species}_${analysis}$threshold_folder/${species}_core_gene_alignment.fasta";
	...
	while(<INGEN>){
		...
		if($line =~ /^>(\S+)/){
			...
			$seq=$1;
			@seq_array=split(//, $seq);
			open INCOOR, "${base_dir}/Analysis/${analysis}/${species}_${analysis}$threshold_folder/${species}_core_genes.tab";
			while(<INCOOR>){
				$line=$_;
				chomp $line;
