...
open OUTPUT, ">${base_dir}/Analysis/${analysis}/${species}_${analysis}/${species}_core_intergenic_alignment.fasta";
...
while(<INGEN>){
	...
	if($line =~ /^>(\S+)/){
		...
		$seq=$1;
		@seq_array=split(//, $seq);
		open INCOOR, "${base_dir}/Analysis/${analysis}/${species}_${analysis}/${species}_core_intergenics.tab";
		while(<INCOOR>){
			$line=$_;
			chomp $line;
